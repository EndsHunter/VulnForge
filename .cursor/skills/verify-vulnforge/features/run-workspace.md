...
The run workspace is the research cockpit for one `target_id/run_id`. A 48px icon rail leads with Home, then Mission, Hunts, Explorer, Report, and Tasks. Footer links leave the run for Settings, Dev, and Tool gaps. Ask is the `#ai-fab` bubble. Evidence is a Report row action. Develop POC is a Report modal, not a rail item.
...
- **Rail labels.** Query `[data-run-rail] [data-nav-id]`. Primary buttons are Mission, Hunts, Explorer, Report, Tasks (`data-nav-id="audit"`). Home is `href="/"`. Footer anchors are Settings (`href="/settings"`), Dev (`href="/dev"`), and Tool gaps (`href="/tool-gaps"`). There is no Evidence, AI, or Develop POC rail item.
...
- **Tasks hash.** Open `#audit/tasks` (or `#tasks/tasks`). The Tasks panel is active (`[data-mode-panel="audit"]`). `[data-nav-id="audit"]` is selected.
...
- Tasks uses mode id `audit`. Assert `[data-mode-panel="audit"]` and `[data-nav-id="audit"]`, not `tasks`. Timeline is a sub-tab inside that panel (`data-tab="timeline"`).
