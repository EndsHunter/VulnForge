...
Hunt mixture-of-agents runs sequential perspectives inside one lease when `stages.hunt_moa` is true (default **off**). See [spikes/2026-09-22-hunt-moa.md](../spikes/2026-09-22-hunt-moa.md). **Settings → Hunt perspectives** writes that flag and `llm.hunt_perspectives` (id and optional model from Available — not `validate_models`). The slot model is a host ref: its URL, API mode, and API key come from that host. Slots share `llm.max_tool_rounds`. Merge inserts at most one candidate and never emits `confirmed`. Provenance is `body.hunt_moa` (a `hunt_moa` note when the cell is none).
