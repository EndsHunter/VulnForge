...
Home lists every audit run under the dashboard runs root, lets the operator search and filter that fleet, and opens New audit, Settings, Dev, AI Chat, and tool gaps.
...
- `home-search` filters rows by target id, run id, or path.
...
- `home-links` reaches Settings, Dev, AI Chat, and tool gaps from Home.

## How to get to it (user POV)

- Open `$VF_VERIFY_URL/`.
- Choose **⌂ Home** from any cockpit header.
- Choose **+ New audit** (`#btn-new-run`).
- Type in **Search runs** (`#home-search`).
- Choose a run row (`a.run-row`) to open that workspace.
- Choose **⚙ Settings**, **Open Dev**, **Open AI Chat**, or **Open tool gaps**.
...
- **Empty fleet.** Open Home before any init. Run `new_tab("$VF_VERIFY_URL/")`. `body` has `data-page="home"`, the heading is `Audit runs`, and `#run-list` contains `No runs yet`. Save `GET /api/runs` as `api-runs-empty.json` with `count` 0. The HTML shell from `GET /` does not include run cards. Those are filled by JS.
...
- **Listed row.** Reload Home. Run `new_tab` only if no Home tab exists, otherwise reload the same tab. `#run-list a.run-row` href is `/runs/<target_id>/<run_id>` and the title contains `toy_sqli`. Screenshot `home-list.png`.
- **Search.** Type the target id into `#home-search`. The matching `a.run-row` stays. A nonsense query shows `No runs match this filter.` Clear the box to restore the row.
...
- **Header links.** `#btn-settings` goes to `/settings`. `#btn-open-dev` goes to `/dev`. `#btn-open-ai-chat` goes to `/chat`. `#btn-open-tool-gaps` goes to `/tool-gaps`. Return to `/` after each hop.
